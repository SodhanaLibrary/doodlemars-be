...
public class BoneVarientsDAO {
	public static ArrayList<VarientDTO> getBoneVarients(String name) throws DBException {
		Connection conn = null;
        PreparedStatement psInsert = null;
        ArrayList<VarientDTO> asrp = new ArrayList<VarientDTO>();
        try {
            conn = DBConn.getConnection();
            psInsert = conn.prepareStatement("select name, image, uuid, json from "+Constants.DB_TABLE_BONE_VARIENTS+" where name = ?");
	...
	public static void addBoneVarients(VarientDTO vdto) throws DBException {
		 Connection conn = null;
	        PreparedStatement psInsert = null;
	        try {
	            conn = DBConn.getConnection();
	            psInsert = conn.prepareStatement("insert into "
	                    + Constants.DB_TABLE_BONE_VARIENTS + "(name, image, uuid, json) values (?,?,?,?)");
	            psInsert.setString(1, vdto.getName());
	            psInsert.setString(2, vdto.getImage());
	            psInsert.setString(3, vdto.getUuid());
	            psInsert.setString(4, vdto.getJson());
