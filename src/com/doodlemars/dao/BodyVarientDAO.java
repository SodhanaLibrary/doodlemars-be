...
public class BodyVarientDAO {
	public static ArrayList<VarientDTO> getBodyVarients(String name) throws DBException {
		Connection conn = null;
        PreparedStatement psInsert = null;
        ArrayList<VarientDTO> asrp = new ArrayList<VarientDTO>();
        try {
            conn = DBConn.getConnection();
            psInsert = conn.prepareStatement("select name, image, uuid, json from "+Constants.DB_TABLE_BODY_VARIENTS+" where name = ?");
	...
	public static void addBodyVarients(VarientDTO vdto) throws DBException {
		 Connection conn = null;
	        PreparedStatement psInsert = null;
	        try {
	            conn = DBConn.getConnection();
	            psInsert = conn.prepareStatement("insert into "
	                    + Constants.DB_TABLE_BODY_VARIENTS + "(name, image, uuid, json) values (?,?,?,?)");
